...
def create_splits_files(root_dir, val_split, test_split):
    ...
    if straify:
        ...
        if type == 1:
            ...
            # stratify
            X_train, X_test, y_train, y_test = train_test_split(images_list, labels, test_size=val_split,
                                                                random_state=42,
            ...
            for name in tqdm(X_test):
                shutil.copy(root_dir.joinpath('images').joinpath(name + '.png'), val_images_dir)
                shutil.copy(root_dir.joinpath('masks').joinpath(name + '.png'), val_masks_dir)
                shutil.copy(root_dir.joinpath('masks_rgb').joinpath(name + '.png'), val_masks_rgb_dir)
            ...
            # stratify
            X_train, X_test, y_train, y_test = train_test_split(x_all, labels_all, test_size=val_split,
                                                                random_state=42,
        ...
        train_FileNames, val_FileNames = np.split(np.array(all_masks), [int(len(all_masks) * (1 - val_split))])
    ...
    if test_split != 0:
        all_masks = get_all_files_in_folder(val_masks_dir, ['*.png'])
        ...
        if straify:
            ...
            if type == 1:
                ...
                for name in tqdm(X_test):
                    shutil.move(val_images_dir.joinpath(name + '.png'), test_images_dir)
                    shutil.move(val_masks_dir.joinpath(name + '.png'), test_masks_dir)
                    shutil.move(val_masks_rgb_dir.joinpath(name + '.png'), test_masks_rgb_dir)

                # check stratification
                all_train_masks = get_all_files_in_folder(val_masks_dir, ['*.png'])
                ...
                for name in tqdm(X_test):
                    shutil.move(str(val_images_dir.joinpath(name + '.png')), str(test_images_dir))
                    shutil.move(str(val_masks_dir.joinpath(name + '.png')), str(test_masks_dir))
                    shutil.move(str(val_masks_rgb_dir.joinpath(name + '.png')), str(test_masks_rgb_dir))
    ...
    all_masks_val = get_all_files_in_folder(val_masks_dir, ['*.png'])
    with open(split_dir.joinpath('val.txt'), 'w') as f:
        for m in all_masks_val:
            f.write(m.stem + '\n')

    all_masks_test = get_all_files_in_folder(test_masks_dir, ['*.png'])
    with open(split_dir.joinpath('test.txt'), 'w') as f:
        for m in all_masks_val:
            f.write(m.stem + '\n')


if __name__ == '__main__':
    create_splits_files(root_dir=Path('denred0_data/data_train_augmentation'), val_split=0.3, test_split=0.5)
